...
try {
    ...
    foreach($test_patients as $patient) {
        ...
        $user_sql = "INSERT INTO users (username, password, role, full_name, email, phone, created_at, updated_at) 
        ...
        mysqli_stmt_bind_param($user_stmt, "sssss", 
            $patient['username'], 
            $hashed_password, 
            $patient['full_name'], 
            $patient['email'], 
        ...
        $patient_sql = "INSERT INTO patients (user_id, date_of_birth, gender, address, medical_history, created_at, updated_at) 
                       VALUES (?, ?, ?, ?, ?, NOW(), NOW())";
        $patient_stmt = mysqli_prepare($conn, $patient_sql);
        
        if (!$patient_stmt) {
            throw new Exception("Error preparing patient insert statement: " . mysqli_error($conn));
        }
        
        mysqli_stmt_bind_param($patient_stmt, "issss", 
            $user_id, 
...
$page_title = "Insert Test Patients";
$current_page = "patients";
require_once "includes/header.php";
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h2>Insert Test Patients</h2>
...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Test Patients Inserted</h5>
        <p class="card-text">
